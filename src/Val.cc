	...
	switch ( val->GetType()->Tag() )
		...
		case TYPE_TABLE:
			...
			for ( const auto& te : *table )
				{
				entry = reinterpret_cast<TableEntryVal*>(te.value);
				k = te.GetHashKey();
...
int TableVal::RecursiveSize() const
	...
	for ( const auto& ve : *v )
		{
		auto* tv = reinterpret_cast<TableEntryVal*>(ve.value);
		if ( tv->GetVal() )
...
bool TableVal::AddTo(Val* val, bool is_first_init, bool propagate_ops) const
	...
	for ( const auto& tble : *tbl )
		...
		auto* v = reinterpret_cast<TableEntryVal*>(tble.value);
...
void TableVal::Describe(ODesc* d) const
	...
	for ( int i = 0; i < n; ++i )
		...
		auto* v = reinterpret_cast<TableEntryVal*>(iter->value);
...
void TableVal::DoExpire(double t)
	...
	for ( int i = 0; i < zeek::detail::table_incremental_step &&
		...
		v = reinterpret_cast<TableEntryVal*>(expire_iterator->value);
...
ValPtr TableVal::DoClone(CloneState* state)
	...
	for ( const auto& tble : *tbl )
		...
		auto* val = reinterpret_cast<TableEntryVal*>(tble.value);
